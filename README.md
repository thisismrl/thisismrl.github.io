...
2. 作品集里可以选择“放进旅行特辑”，用于把旅行照片统一收在前台 Works 的旅行特辑下面。
3. 进入 `/admin/collections` 可以拖动作品集，或用“上移 / 下移”调整旅行特辑在前台的顺序。
4. 进入作品集编辑页，在下方照片管理区域上传和整理照片。
5. 可以一次选择一张或多张照片。
6. 多张照片会自动接在该作品集现有排序后面。
7. 可以把第一张上传照片自动设为作品集封面，也可以在每张照片旁勾选“作品集封面”。
8. 上传后会保留原图到 `uploads/originals/`。
9. 同时生成前台展示图到 `uploads/display/`，封面图到 `uploads/covers/`。
10. 前台和静态导出只使用 WebP 展示图，不直接加载原图。
...
- `Fiction`：小说或故事。标题必填，需要封面，摘要可手写；如果摘要为空，系统会从正文自动截取。
- `Essay`：随笔。封面会被忽略；前台以左侧时间、右侧文字的时间流方式展示。
- `Notes`：笔记、短札或日记性文字。封面会被忽略；标题可以为空，系统会用发布时间生成标题；摘要为空时会从正文自动生成，作为 News 和文字列表里的短句。
- `Poem`：诗歌。封面会被忽略；前台以左侧时间、右侧文字的时间流方式展示。
